    ...
    while True:
        with picamera.PiCamera() as camera:
            with picamera.array.PiRGBArray(camera) as output:
                ...
                outlines = ProcessImage1(output.array)
                output.truncate(0)
                ...
                for t in TrackList:
                    ...
                    cv.putText(output.array,str(t.id)+"_"+str(t.color),coord, font, 1,(0,255-t.count*25,50),2,cv.LINE_AA)                
                out_img = cv.resize(output.array, (640,480), interpolation= cv.INTER_LINEAR) 
